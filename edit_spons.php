<?php

$mysqli = mysqli_connect("localhost", "root", "", "atelier_php");
$name = $_GET['name'];


//$sql = "SELECT *  from event2 where name='$name'";

//$result = $conn->query($sql);

//echo"$result" ; 

$query = "SELECT *  from sponsor where name='$name'";

$result = mysqli_query($mysqli, $query);

/* fetch associative array */
$row = mysqli_fetch_row($result); 
$id =  $row[0] ; 
/*
echo "$row[0]" ; 
echo "$row[1]" ; 

echo "$row[2]" ; 
echo "$row[3]" ; 
*/
if(isset($_POST["description"])&& isset($_POST["name"])&& isset($_POST["image"]))
{
    $description = $_POST["description"] ; 
    
    $name = $_POST["name"];   
    $image = $_POST["image"];

    echo"$id";
    echo"$name" ; 
    echo"$image" ; 
    echo"$description" ; 

    $sql = "update sponsor set name='$name',description='$description',image='$image' where id=$id" ;
    $mysqli->query($sql);
    $mysqli->close() ;
    header("location: view_sponsor.php");


}





$mysqli->close() ; 


//$conn->close();

//header("location: test2.php");

?>

<html>


<head>
    <!-- Required meta tags-->
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Colorlib Templates">
    <meta name="author" content="Colorlib">
    <meta name="keywords" content="Colorlib Templates">

    <!-- Title Page-->
    <title>Au Register Forms by Colorlib</title>

    <!-- Font special for pages-->
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i,800,800i" rel="stylesheet">

    <!-- Vendor CSS-->

    <!-- Main CSS-->
    <link href="create.css" rel="stylesheet" media="all">
</head>

<body>
    <form class="form-inline m-2"  method="POST">  
        <div class="page-wrapper bg-gra-03 p-t-45 p-b-50">
            <div class="wrapper wrapper--w790">
                <div class="card card-5">
                    <div class="card-heading">
                        <h2 class="title">Event Update Registration Form</h2>
                    </div>
                    
                    <div class="card-body">
                        <form method="POST">
                            
                            <div class="form-row m-b-55">
                                <div class="name">Name </div>
                                <div class="value">
                                    <div class="row row-space">
                                        <div class="col-2">
                                            <div class="input-group-desc">
                                                
                                                <input class="input--style-5" value="<?php echo"$row[1]"?>" type="text" name="name">
                                                
                                            </div>
                                        </div>
                                        
                                    
                                    </div>
                                </div>
                            </div>


                            <div class="form-row">
                                <div class="name">Description</div>
                                <div class="value">
                                    <div class="input-group">
                                        <textarea  name="description" class="input--style-5"  cols="40" rows="2"> <?php echo"$row[2]" ?></textarea>
                                        
                                    </div>
                                </div>
                            </div>


                            <div class="form-row m-b-55">
                                <div class="name">Image</div>
                                <div class="value">
                                    <div class="row row-space">
                                        <div class="col-2">
                                            <div class="input-group-desc">
                                                
                                                <input class="input--style-5" value="<?php echo"$row[3]" ?>"  type="text" name="image">
                                                
                                            </div>
                                        </div>
                                        
                                    </div>
                                </div>
                            </div>

                            
                            <div>
                            <!--    <button class="btn btn--radius-2 btn--red" type="submit">Register</button> -->
                            
                                <button href="view_sponsor.php" class="btn btn--radius-2 btn--red" type="submit">Update</button>
                        
                            </div>
                    
                    
                        </form>
                
                    </div>
                
                
                </div>
            </div>
        </div>

    </form> 

</body>











</html>

